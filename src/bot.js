...
// Removed fs, path, os as they are not used in this reverted version.
import {
  ...
  try {
    ...
    if (result.success && result.subtitle && result.subtitle.url) {
      ...
      if (subtitleContent) {
        ...
        bot.sendDocument(chatId, Buffer.from(subtitleContent), {}, { filename: filename, contentType: 'text/plain' });
      } else {
...
bot.on('polling_error', (error) => {
  const pollingErrorMsg = error.code ? (error.code + ' ' + error.message) : JSON.stringify(error);
  console.error('Bot Polling Error:', pollingErrorMsg);
});
bot.on('webhook_error', (error) => {
  const webhookErrorMsg = error.code ? (error.code + ' ' + error.message) : JSON.stringify(error);
  console.error('Bot Webhook Error:', webhookErrorMsg);
});
