      ...
      (success, response) => {
        ...
        try {
          ...
          subtitles = subtitles.reduce((result, yaSubtitlesObject) => {
            if (yaSubtitlesObject.language && !result.find(e => e.source === "yandex" && e.language === yaSubtitlesObject.language && !e.translatedFromLanguage)) {
              result.push({ source: "yandex", language: yaSubtitlesObject.language, url: yaSubtitlesObject.url, type: 'original' });
            }
            if (yaSubtitlesObject.translatedLanguage) {
              result.push({ source: "yandex", language: yaSubtitlesObject.translatedLanguage, translatedFromLanguage: yaSubtitlesObject.language, url: yaSubtitlesObject.translatedUrl, type: 'translated' });
            }
            return result;
          }, []);
